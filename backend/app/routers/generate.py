...
from app.utils.helpers import code_parses_base64_upload_as_json
...
def _validate_generated_code_shape(*, code: str, schema_obj: Any, file_kind: str) -> None:
    ...
    if code_parses_base64_upload_as_json(code):
        raise ValueError("generated code must not JSON.parse the raw base64 upload (file is not JSON text)")

    if file_kind in {"pdf", "docx", "txt", "md", "rtf", "odt", "xml", "epub", "fb2", "doc", "unknown"}:
