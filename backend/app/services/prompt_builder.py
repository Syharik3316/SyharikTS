...
def build_generation_prompt(
    ...
    *,
    ...
    return (
        ...
        "Hard requirements:\n"
        "1) Keep interface and function signature exactly.\n"
        "2) If base64file is null/undefined/empty after trim, return [].\n"
        "3) Use EXTRACTED_INPUT_JSON as source-of-truth (already extracted server-side).\n"
        "4) Preserve top-level schema shape exactly (including nested arrays/objects and input:[]).\n"
        ...
        "5) Never collapse arrays/objects to scalar strings like {input: ''}.\n"
        "6) Implement recursive cast by schema example types: string, number, boolean, null, object, array.\n"
        "7) Prefer data from extracted.records, then extracted.tables rows, then extracted.text hints.\n"
        "7.1) Use Header aliases map for semantic matching when source keys differ from schema keys.\n"
        "7.2) For checkbox-like values, treat markers [X, x, ☑, ☒, ✓] as selected.\n"
        "8) Unknown fields should not break output; fill defaults by schema shape.\n"
        "9) STRICT KEYS MODE: output keys must be ONLY from user schema. Never add extra keys not present in schema.\n"
        "10) If schema is from array example, still use ONLY keys from the first object and do not invent missing fields.\n"
        "11) Key matching strictness: for short keys/aliases (<= 8 chars after normalization), allow ONLY exact normalized match; never substring match.\n"
        "12) No `as any`, no markdown, no explanation text.\n"
        "Schema object:\n"
        f"{schema_compact}\n"
        "Schema keys:\n"
